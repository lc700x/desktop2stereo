...
FRAGMENT_SHADER = """
    ...
    void main() {
        ...
        vec4 col = texture(tex_color, offset_uv);

        // Depth-guided hole filling
        if (col.a < 0.01) {  // missing pixel
            vec2 texel = 1.0 / textureSize(tex_color, 0);
            float best_depth = 1.0;
            vec4 best_col = vec4(0.0);
            // 3x3 neighborhood search
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    vec2 neighbor_uv = offset_uv + vec2(dx, dy) * texel;
                    if (neighbor_uv.x < 0.0 || neighbor_uv.x > 1.0 ||
                        neighbor_uv.y < 0.0 || neighbor_uv.y > 1.0) continue;
                    float n_depth = texture(tex_depth, neighbor_uv).r;
                    vec4 n_col = texture(tex_color, neighbor_uv);
                    if (n_col.a > 0.01 && n_depth < best_depth) {
                        best_depth = n_depth;
                        best_col = n_col;
                    }
                }
...
class StereoWindow:
    """Optimized stereo viewer with depth-guided hole filling"""
    ...
    def on_key_event(self, window, key, scancode, action, mods):
        if action == glfw.PRESS:
            if key in (glfw.KEY_ENTER, glfw.KEY_SPACE):
                ...
                self.depth_ratio = max(0, self.depth_ratio - 1.0)
            elif key == glfw.KEY_UP:
                self.depth_ratio = min(20, self.depth_ratio + 1.0)
            elif key == glfw.KEY_0:
    ...
    def update_frame(self, rgb, depth):
        ...
        if self._texture_size != (w, h):
            ...
            self.color_tex = self.ctx.texture((w, h), 4, dtype='f1')  # RGBA for hole fill
    ...
    def render(self):
        ...
        if self.display_mode == "Full-SBS":
            ...
            center_y = win_h / 2.0
            self.ctx.viewport = (int(win_w / 4.0 - render_w / 2),
                                 int(center_y - render_h / 2), render_w, render_h)
            self.prog['u_eye_offset'].value = -self.ipd_uv / 2.0
            self.quad_vao.render(moderngl.TRIANGLE_STRIP)
            self.ctx.viewport = (int(3 * win_w / 4.0 - render_w / 2),
                                 int(center_y - render_h / 2), render_w, render_h)
            self.prog['u_eye_offset'].value = self.ipd_uv / 2.0
            self.quad_vao.render(moderngl.TRIANGLE_STRIP)
            ...
            render_w, render_h = self._compute_render_size(max_w, max_h, src_w, src_h)
            self.ctx.viewport = (int(win_w / 2.0 - render_w / 2),
                                 int(win_h / 4.0 - render_h / 2), render_w, render_h)
            self.prog['u_eye_offset'].value = -self.ipd_uv / 2.0
            self.quad_vao.render(moderngl.TRIANGLE_STRIP)
            self.ctx.viewport = (int(win_w / 2.0 - render_w / 2),
                                 int(3 * win_h / 4.0 - render_h / 2), render_w, render_h)
            self.prog['u_eye_offset'].value = self.ipd_uv / 2.0
            self.quad_vao.render(moderngl.TRIANGLE_STRIP)
