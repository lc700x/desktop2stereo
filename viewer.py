...
if USE_3D_MONITOR:
    class StereoWindow:
        ...
        def __init__(self, ipd=0.064, depth_ratio=1.0, display_mode="Half-SBS", keep_ratio=KEEP_RATIO, show_fps=True):
            ...
            # Configure window for borderless fullscreen
        ...
        def position_on_monitor(self, monitor_index=0):
            ...
            if monitor_index < len(monitors):
                ...
                mon_x, mon_y = glfw.get_monitor_pos(monitor)
                vidmode = glfw.get_video_mode(monitor)
                width, height = vidmode.size.width, vidmode.size.height
        ...
        def get_current_monitor(self):
            """Get current monitor the window is on"""
            monitors = glfw.get_monitors()
            if not monitors:
                return None
        ...
        def on_key_event(self, window, key, scancode, action, mods):
            ...
            if action == glfw.PRESS:
                ...
                elif key == glfw.KEY_F:  # Toggle FPS display
