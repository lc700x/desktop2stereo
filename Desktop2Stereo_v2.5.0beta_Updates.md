...
- **XR Glow 光效增强与状态 OSD** — Glow 现在支持基于屏幕内容的动态源取样与 mipmap 采样，颜色更贴合画面；新增光效状态 OSD，可显示当前 Glow/Veil/Mist 状态与光效透明度调整反馈。
- **OpenXR 异步 GPU/PBO 渲染优化** — `FrameMixin` 新增 glow mipmap 异步管理；`OpenXRViewer` 使用颜色/深度 PBO ring 管理 GPU interop 与 CPU fallback 上传路径，减少同步等待并提升帧率稳定性。
- **XR 视频裁剪模式（自动/手动/关闭）** — GUI 新增裁剪模式下拉菜单。`自动`沿用黑边检测，`手动`可用左手柄手势自定义裁剪矩形，`关闭`禁用裁剪。手动模式下：左扳机（激光不指屏幕）按住 3 秒循环裁剪模式，双击开关裁剪调整暂停；调整时左摇杆按主轴裁剪（X=左右，Y=上下），裁剪只缩小该轴屏幕尺寸，不改变屏幕距离。
...
- **XR 远程输入快捷键更新** — 左摇杆无握持时改为键盘上下左右方向键，并采用累积重复触发；右摇杆无握持时保留鼠标水平/垂直滚动。
...
| 19 | 调整深度强度 / 光效透明度 | 右握持 + 左摇杆 Y / X 轴 | `input_handler.py:1575-1654` |
...
| 25 | 键盘方向键 | 左摇杆 X/Y（不握持）：上下左右方向键 | `input_handler.py:1658-1661` |
| 26 | 鼠标水平/垂直滚动 | 右摇杆 X/Y（不握持，不单击摇杆） | `input_handler.py:1769-1771` |
...
| 41 | 循环裁剪模式（自动/手动/关闭） | 左扳机按住 3 秒（激光不指屏幕） | `input_handler.py:1013-1025` |
| 42 | 开关裁剪调整暂停 | 左扳机双击（激光不指屏幕，手动模式） | `input_handler.py:1027-1043` |
| 43 | 调整裁剪矩形 | 左摇杆（裁剪调整中，不握持）：X=左右，Y=上下，仅主轴 | `input_handler.py:1480-1494` |
...
- **XR glow enhancements and light OSD** — Glow now supports dynamic screen-content source sampling and mipmap sampling for better color matching; added a light-effect OSD showing the current Glow/Veil/Mist state and light effect opacity adjustment feedback.
- **OpenXR async GPU/PBO rendering optimization** — `FrameMixin` now manages glow mipmap updates asynchronously; `OpenXRViewer` uses color/depth PBO rings for GPU interop and CPU fallback upload paths to reduce synchronization stalls and improve FPS stability.
- **XR video crop modes (Auto/Manual/Off)** — New Crop Mode dropdown in the GUI. `Auto` keeps the existing letterbox detection, `Manual` lets you set a custom crop rectangle with left-controller gestures, and `Off` disables cropping. In Manual mode: left trigger (laser off screen) held 3s cycles crop mode, double-tap toggles crop-adjust pause; while adjusting, the left stick crops the dominant axis (X = sides, Y = top/bottom). Cropping only shrinks that axis of the screen and never changes screen distance.
...
- **XR Preview** — Toggleable preview window for OpenXR mode, independent of the headset display.
- **XR remote input shortcut update** — Left stick without grip now sends keyboard Up/Down/Left/Right arrows with accumulated repeat; right stick without grip keeps horizontal/vertical mouse scrolling.
...
| 19 | Adjust Depth Intensity / Light Effect Opacity | Right Grip + Left Stick Y / X | `input_handler.py:1575-1654` |
...
| 25 | Keyboard Arrow Keys | Left Stick X/Y (no grip): Up/Down/Left/Right arrows | `input_handler.py:1658-1661` |
| 26 | Horizontal / Vertical Mouse Scroll | Right Stick X/Y (no grip, no stick click) | `input_handler.py:1769-1771` |
...
| 41 | Cycle Crop Mode (Auto/Manual/Off) | Left Trigger held 3s (laser off screen) | `input_handler.py:1013-1025` |
| 42 | Toggle Crop-Adjust Pause | Left Trigger double-tap (laser off screen, Manual mode) | `input_handler.py:1027-1043` |
| 43 | Adjust Crop Rectangle | Left Stick (crop-adjust active, no grip): X = sides, Y = top/bottom, dominant axis only | `input_handler.py:1480-1494` |
