...
### 新功能
- **MIGraphX 加速支持** — 通过 Intel MIGraphX 后端为 AMD GPU 提供推理加速，GUI 新增独立复选框（`MIGraphX` + `重编译 MIGraphX`）。
- **InfiniDepth 模型支持** — 新增 InfiniDepth 作为深度估计选项。
- **Depth-Anything-V1 室内外变体** — 新增针对不同场景的 DA-V1 室内/室外变体模型，提升深度精度。
- **Mac ScreenCaptureKit** — macOS 上以现代 ScreenCaptureKit API 替换旧的 Quartz 捕获方式，性能和稳定性显著提升。
- **虚拟环境模型选择** — XR 模式新增环境下拉菜单，支持"默认"（黑色背景）和用户自定义的 `xr_viewer/environments/` 中 3D 场景，支持从 `profile.json` 读取本地化显示名称。
...
### New Features
- **MIGraphX acceleration support** — AMD GPU inference via Intel MIGraphX backend, with dedicated checkboxes (`MIGraphX` + `Recompile MIGraphX`) in the GUI.
...
- **Mac ScreenCaptureKit** — Replaced legacy Quartz capture with modern ScreenCaptureKit API on macOS for better performance and stability.
- **Virtual environment model selection** — Added environment dropdown in XR mode; supports "Default" (black backdrop) and user-defined 3D scenes from `xr_viewer/environments/` with localized display names from `profile.json`.
...
| `curved` | bool | `false` | Whether the screen starts in curved mode. |
