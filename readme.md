...
4. Install Python environment
    ...
    **Older AMD GPU**: Run [install-rocm.bash](http://_vscodecontentref_/0):

    ```bash
    bash install-rocm.bash
    ```

    **NVIDIA GPU**: Run [install-cuda.bash](http://_vscodecontentref_/1):
...
> - Other `RTSP`, [RTMP](http://_vscodecontentref_/2), `HLS M3U8` protocol may be chosen for **VLC Player** [i.e. extend screen mode for AR glasses] / VR Video Apps (**DeoVR**) on client devices.
...
> Tip: **MJPEG Streamer** mode is wireless streaming with video only to client devices/apps with lower latency, like **Wolvic Browser**, etc. For VR or Huawei AR: [Wolvic Browser (Chromium Based)](https://wolvic.com/dl/) is recommended to open the HTTP MJPEG link.
...
> Tip: **3D Monitor** mode is a special **Local Viewer** mode dedicated for a 3D Monitor, no virtual display driver needed for this mode. It can only run as **fullscreen** and be used **locally** as the screen capture attribute for the **Stereo Viewer** window is `disabled` globally.
