...
def _cc_cmd(cc, src, out, include_dirs, library_dirs, libraries):
    _basename = os.path.basename(cc).lower()
    if _basename in ("cl", "cl.exe") or "cl.exe" in _basename or "clang-cl" in cc or "icx-cl" in cc:
...
def _build(name: str, src: str, srcdir: str, library_dirs: list[str], include_dirs: list[str], libraries: list[str],
    ...
    if is_xpu():
        ...
        if cxx is icpx:
            ...
            if os.environ.get("TRITON_SUPPRESS_GCC_HOST_CODE_DEPRECATION_WARNINGS", "1") == "1":
                if "cl" in os.path.basename(cxx).lower():
                    ccflags += []  # MSVC already uses /wd4996 in _cc_cmd
                else:
    ...
    # When using MSVC cl.exe, initialize the VS environment so it can find
    # Windows SDK headers and libraries. Must run in a single shell so the
    # environment variables set by the batch file persist for cl.exe.
    env = os.environ.copy()
    _basename = os.path.basename(cc).lower() if cc else ""
    if _basename in ("cl", "cl.exe") or "cl.exe" in _basename:
        ...
        if os.path.exists(vcvarsall):
            ...
            init = f'call "{vcvarsall}" x64 >nul &&'
            cmdline = sp.list2cmdline(cc_cmd)
            full_cmd = f'{init} {cmdline}'
            subprocess.check_call(
                full_cmd,
                shell=True,
                env=env,
                stdout=None if os.getenv("VERBOSE") else subprocess.DEVNULL,
            )
            return so

    subprocess.check_call(cc_cmd, stdout=subprocess.DEVNULL, env=os.environ.copy())
    return so
