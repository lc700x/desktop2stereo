...
|---|---|---|
| **屏幕控制** |||
| 左/右手握持键 (按住) | 拖拽屏幕 | 使用激光锚点移动屏幕 |
| 右握持 + 右摇杆 X | 左右 | 调整屏幕宽度 |
| 右握持 + 右摇杆 Y | 上下 | 调整屏幕距离 (加速) |
| 右摇杆按键 (短按) | 切换 | 平面 / 曲面屏幕 |
| 右摇杆按键 (长按) | 重置 | 重置屏幕方向 (保持距离/大小) |
| 左握持 + 左摇杆 X/Y | 推方向 | 平移屏幕位置 |
| 左握持 + 右摇杆 X | 左右 | 屏幕水平旋转 (Yaw) |
| 左握持 + 右摇杆 Y | 上下 | 屏幕倾斜 (Pitch) |
| 左 Y 键 (短按) | 重置 | 重置屏幕位置与朝向 |
| 左 Y 键 (长按) | 切换 | 切换屏幕预设 |
| 左摇杆按键 (短按) | 切换 | 切换背景颜色 (5种) |
| 左摇杆按键 (长按) | 切换 | FPS / 帮助面板叠加层 |
| 左 X 键 (轻按 <1s) | 切换 | 显示 / 隐藏虚拟键盘 |
| 左 X 键 (按住 >1s 后释放) | 切换 | 切换普通光效 / 发光模式 |
| 左 X 键 (按住 >4s) | 切换 | VDXR 透视模式 |
| 左菜单键 (短按) | 切换 | FPS 叠加层 |
| 左菜单键 (长按) | 重置 | 重置深度比例为默认值 |
| 右握持 + 左摇杆 L/R | 左右 | 调整光效透明度 (0–1) |
| **手动裁切** |||
| 左扳机 (激光指向屏幕外, 按住3秒) | 切换 | 裁切模式: 自动 / 手动 / 关闭 |
| 左扳机 (屏幕外, 手动模式, 双击) | 切换 | 裁切暂停 (冻结画面 + OSD) |
| 左摇杆 (裁切调整中, 无握持) | 方向 | 裁切边 (X) 或 顶/底 (Y), 仅主轴生效 |
| **深度调节** |||
| 右握持 + 左摇杆 Y | 上下 | 调整深度比例 (0–10) |
| 右握持 + 左摇杆 X | 左右 | 调整光效透明度 (0–1) |
...
| Shift / Ctrl / Alt / Win | 单击 / 双击 | 瞬时 / 锁定修饰键 |
| 左握持 + 左摇杆 (指向键盘) | 方向 | 平移键盘位置 |
| 右握持 + 右摇杆 (指向键盘) | 方向 | 调整键盘大小/距离 |
| **手柄模型** |||
| 右 A + B (按住0.5秒) | 切换 | 切换手柄模型品牌 |
| 左菜单 + 右 A + B (按住1秒) | 切换 | 进入/退出手柄校准模式 |

#### 裁切模式 (OpenXR串流)

裁切模式移除捕获画面中不需要的边框 (如电影的黑边), 使视频填满虚拟屏幕。可在 GUI 的 **裁切模式** 下拉菜单中选择模式, 或在 VR 中循环切换。

- **自动** — 内容变化时自动检测并移除上下/左右黑边。最适合带有固定黑边的影片。
- **手动** — 使用左手手柄设置自定义裁切矩形。当自动裁切误判或需要特定构图时使用。
- **关闭** — 不裁切; 显示完整捕获画面。

**手动裁切手势** (左手手柄, 激光指向屏幕外):

1. **按住左扳机3秒** 循环 `自动 → 手动 → 关闭`。屏幕上有指示器显示当前模式。
2. **手动模式**下, **双击左扳机** 进入裁切调整 (画面冻结并显示像素尺寸 OSD)。再次双击应用并恢复。
3. 调整时推动 **左摇杆**: **左右 (X)** 裁切两侧, **上下 (Y)** 裁切顶部/底部。每次移动仅应用主摇杆方向, 不会同时裁切两个轴。裁切会缩小该轴的屏幕尺寸而不改变屏幕距离。

手动裁切矩形和所选裁切模式随运行时设置一起保存。
