...
if OS_NAME == "Windows":
    ...
    def add_mouse(img): # TODO: add mouse cursor on mac
        return img
else:
    def add_mouse(img):
        return img
...
class DesktopGrabber:
    def __init__(self, monitor_index=1, output_resolution=1080, show_monitor_info=True, fps=60, backend="mss"):
        ...
        if self.backend == "bettercam":
            import bettercam
            self.camera = bettercam.create(output_idx=self.monitor_index-1, output_color="RGB") 
    ...
    def grab(self):
        ...
        if self.backend == "bettercam":
            img = self.camera.get_latest_frame()  # Waits for a new frame
            img_array = add_mouse(img)
        else:
